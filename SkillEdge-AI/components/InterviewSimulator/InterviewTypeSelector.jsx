...
export default function InterviewTypeSelector() {
  ...
  const handleResumeUpload = async () => {
    ...
    // Directly redirect to interview with position
  ...
  return (
    <div
      className={`min-h-screen bg-gradient-to-br from-gray-900 via-indigo-900 to-black text-white flex flex-col items-center justify-center px-4 transition-opacity duration-1000 ${isLoaded ? "opacity-100" : "opacity-0"
        }`}
    >
        ...
        {interviewTypes.map((type) => {
          ...
          return (
            <div
              ...
              className={`cursor-pointer transform transition-all duration-500 hover:scale-105 backdrop-blur-md bg-gray-800/70 border-2 rounded-2xl p-8 shadow-xl ${isSelected
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {technicalRoles.map((role) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg transition-all duration-200 border ${selectedRole === role
      ...
      {selectedType === "resume" && (
        <div className="mb-10 text-center space-y-6">
          <div className="max-w-md mx-auto">
            <h3 className="text-xl font-semibold mb-3 text-purple-300">Select Position:</h3>
            <select
              value={position}
              onChange={(e) => setPosition(e.target.value)}
              className="w-full px-4 py-2 rounded-lg bg-gray-800 text-white border border-gray-600 focus:border-purple-500 focus:ring-2 focus:ring-purple-500 focus:outline-none"
            >
          ...
          <div className="max-w-md mx-auto">
            <h3 className="text-xl font-semibold mb-3 text-purple-300">Upload Your Resume:</h3>
      ...
      <div className="text-center">
        <button
          ...
          className={`px-8 py-3 rounded-xl text-white font-medium transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 ${selectedType === "resume" && !resumeFile
